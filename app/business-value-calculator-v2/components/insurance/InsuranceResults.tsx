...
type InsuranceResultsData = ReturnType<
  typeof calculateInsuranceAssessment
>;

type Props = {
  inputs: InsuranceInputs;
  results: InsuranceResultsData;
};
...
export default function InsuranceResults({
  ...
  const exportJson = () => {
    ...
    link.href = url;
    link.download =
      `DDC-Insurance-Assessment-${new Date()
        .toISOString()
        .slice(0, 10)}.json`;
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <Card index={2} title="Estimated DDC implementation scope">
          <div className="space-y-2 text-sm text-slate-200">
            <div>{results.scope.businessUnitsSites} business units / operating entities</div>
            <div>
            ...
            <div>
              {results.scope.integrationGroups} claim / assessment integration groups
            </div>
        ...
        <Card index={10} title="Secondary claim review & dispute benefit">
        ...
        <Card index={11} title="Systems requiring replacement">
          ...
          <div className="mt-2 text-sm leading-6 text-slate-400">
            Existing policy, claims, document-management, fraud, assessment, payment and workflow systems remain in place. DDC adds an independent verifiable operational record layer.
          </div>
        </Card>
      </div>
