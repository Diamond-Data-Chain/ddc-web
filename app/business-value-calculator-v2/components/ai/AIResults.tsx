...
type AIResultsData =
  ReturnType<typeof calculateAIAssessment>;
...
export default function AIResults({
  ...
  const exportJson = () => {
    ...
    link.href = url;
    link.download =
      `DDC-AI-Digital-Systems-Assessment-${new Date()
        .toISOString()
        .slice(0, 10)}.json`;
  ...
  return (
    <section className="rounded-[30px] border border-amber-400/40 bg-slate-900/75 p-6 shadow-[0_0_50px_rgba(251,191,36,0.08)] md:p-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-semibold">
            AI & Digital Systems assessment results
          </h2>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="rounded-xl border border-amber-300 bg-amber-400/20 px-4 py-2 text-sm font-semibold text-amber-100"
          >
          ...
          <button
            type="button"
            onClick={exportJson}
            className="rounded-xl border border-slate-700 bg-slate-950/70 px-4 py-2 text-sm font-semibold text-slate-300"
          >
            Export JSON
          </button>
        </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <Card index={1} title="Estimated current addressable operational cost">
          <div className="text-3xl font-semibold text-white">
            {money.format(results.currentCost.totalAddressableOperationalCostUsdt)}
          </div>
          <div className="mt-1 text-sm text-slate-400">per year</div>
        </Card>

        <Card index={2} title="Estimated DDC implementation scope">
          <div className="space-y-2 text-sm text-slate-200">
            <div>{results.scope.operationalEnvironments} operational environments</div>
            <div>
              {results.scope.standardConnectors +
                results.scope.customConnectors} systems/connectors
            </div>
            <div>{results.scope.integrationGroups} model / agent integration groups</div>
            <div>{results.scope.workflows} governance workflows</div>
            <div>{number.format(results.scope.annualRegistrationTransactions)} records/year</div>
          </div>
        </Card>
        ...
        <Card index={3} title="Estimated implementation team & effort">
          ...
          <div className="mt-4 space-y-2 text-sm text-slate-300">
            {results.implementation.roleBreakdown
              ...
              .map((role) => (
                <div key={role.role} className="flex justify-between gap-3">
        ...
        <Card index={4} title="Estimated one-time DDC implementation cost">
          <div className="text-3xl font-semibold text-amber-200">
            {money.format(results.implementation.estimatedImplementationCostUsdt)}
          </div>
        </Card>

        <Card index={5} title="Estimated annual DDC operating cost">
          <div className="text-3xl font-semibold text-white">
            {money.format(results.annualOperations.estimatedAnnualOperatingCostUsdt)}
          </div>
          <div className="mt-1 text-sm text-slate-400">per year</div>
        </Card>
        ...
        <Card index={6} title="Estimated DDC network requirement">
          ...
          <div className="mt-2 text-sm text-slate-300">
            {money.format(results.annualOperations.networkCostUsdt)} / year at 0.79 USDT/DDC assessment reference
          </div>
        </Card>

        <Card index={7} title="Estimated recurring annual benefit">
          <div className="text-3xl font-semibold text-emerald-300">
            {money.format(results.value.recurringAnnualBenefitUsdt)}
          </div>
          <div className="mt-1 text-sm text-slate-400">per year</div>
        </Card>
        ...
        <Card index={10} title="Secondary AI decision review benefit">
          <div className="space-y-2 text-sm text-slate-300">
            <div>
              Current review labor:{" "}
              <strong className="text-white">
                {number.format(results.reconstruction.currentLaborHours)} h/year
              </strong>
            </div>
            <div>
              Target review labor:{" "}
              <strong className="text-white">
                {number.format(results.reconstruction.targetLaborHours)} h/year
              </strong>
            </div>
            <div>
              Assumption:{" "}
        ...
        <Card index={11} title="Systems requiring replacement">
          ...
          <div className="mt-2 text-sm leading-6 text-slate-400">
            Existing model registries, MLOps platforms, agent systems,
            observability, approval workflows, governance platforms and
            data systems remain in place. DDC adds an independent
            verifiable decision-governance record layer.
          </div>
        </Card>
      </div>
      ...
      <div className="mt-8 rounded-2xl border border-cyan-400/20 bg-cyan-400/[0.06] p-5">
        ...
        <div className="mt-3 grid gap-2 text-sm text-slate-300 md:grid-cols-2">
          <div>Staff record-handling reduction: {results.assumptions.staffRecordHandlingReductionPercent}%</div>
          <div>Other record-management reduction: {results.assumptions.otherRecordManagementReductionPercent}%</div>
          <div>AI review / investigation labor reduction: {results.assumptions.reviewLaborReductionPercent}%</div>
          <div>Planning fee: {results.assumptions.feePerRegistrationDdc} DDC/registration</div>
          <div>DDC assessment reference: {results.assumptions.ddcReferencePriceUsdt} USDT/DDC</div>
        </div>
      </div>
