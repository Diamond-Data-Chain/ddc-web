...
type OtherResultsData =
  ReturnType<typeof calculateOtherAssessment>;

type Props = {
  inputs: OtherInputs;
  results: OtherResultsData;
};
...
export default function OtherResults({
  ...
  const exportJson = () => {
    ...
    link.href = url;
    link.download =
      `DDC-Other-Operational-Assessment-${new Date()
        .toISOString()
        .slice(0, 10)}.json`;
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-semibold">
            Custom operational assessment results
          </h2>
        </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <Card index={2} title="Estimated DDC implementation scope">
          <div className="space-y-2 text-sm text-slate-200">
            <div>
              {results.scope.operationalSites} operational sites / business units
            </div>
            ...
            <div>
              {results.scope.integrationGroups} operational integration groups
            </div>

            <div>
              {results.scope.workflows} workflows
            </div>
        ...
        <Card index={6} title="Estimated DDC network requirement">
          ...
          <div className="mt-2 text-sm text-slate-300">
            {money.format(
              results.annualOperations.networkCostUsdt
            )}{" "}
            / year at 0.79 USDT/DDC assessment reference
          </div>
        </Card>
        ...
        <Card index={8} title="Estimated first-year net value">
          <div className="text-3xl font-semibold text-white">
            {money.format(
              results.value.firstYearNetValueUsdt
            )}
          </div>
        </Card>
        ...
        <Card index={10} title="Secondary operational review benefit">
        ...
        <Card index={11} title="Systems requiring replacement">
          ...
          <div className="mt-2 text-sm leading-6 text-slate-400">
            Existing operational, workflow, document, data and other business
            systems remain in place. DDC adds an independent verifiable
